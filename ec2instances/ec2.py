    ...
    for product_item in product_iterator:
        for offer_string in product_item.get('PriceList'):
            ...
            try:
                ...
                print(f"ERROR: Unknown OS or Software. Exception adding pricing for {instance_type}: {e}")
...
def parse_instance(instance_type, product_attributes, api_description):
    ...
    if api_description:
        ...
        try:
            if api_description['EbsInfo']['EbsOptimizedSupport'] != 'unsupported':
                i.ebs_baseline_bandwidth = float(api_description['EbsInfo']['EbsOptimizedInfo']['BaselineBandwidthInMbps'])
                i.ebs_baseline_throughput_mbps = float(api_description['EbsInfo']['EbsOptimizedInfo']['BaselineThroughputInMBps'])
                i.ebs_baseline_iops = float(api_description['EbsInfo']['EbsOptimizedInfo']['BaselineIops'])
        except:
            print(f'Error Parsing api description: {api_description}')
